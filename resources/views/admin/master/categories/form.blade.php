...
@section('content')
    <form action="{{ $category->exists ? route('admin.master.categories.update', $category) : route('admin.master.categories.store') }}" method="POST" class="max-w-3xl space-y-6">
        @csrf
        ...
        <div class="bg-white rounded-2xl border border-slate-100 p-6 grid sm:grid-cols-2 gap-4">
            ...
            <div>
                <label class="block text-xs font-bold text-ink-muted mb-1">Banner Image</label>
                <input name="banner_image" value="{{ old('banner_image', $category->banner_image) }}" class="w-full px-3 py-2 rounded-xl border border-slate-200">
            </div>
            <div>
                <label class="block text-xs font-bold text-ink-muted mb-1">Card Image</label>
                <input name="card_image" value="{{ old('card_image', $category->card_image) }}" class="w-full px-3 py-2 rounded-xl border border-slate-200">
            </div>
            <div>
