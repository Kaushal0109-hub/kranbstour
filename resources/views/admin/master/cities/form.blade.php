...
@section('content')
    <form action="{{ $city->exists ? route('admin.master.cities.update', $city) : route('admin.master.cities.store') }}" method="POST" enctype="multipart/form-data" class="max-w-3xl space-y-6">
        @csrf
        ...
        <div class="bg-white rounded-2xl border border-slate-100 p-6 grid sm:grid-cols-2 gap-4">
            ...
            <div>
                <x-admin-image-field name="card_image" label="Card Image" :value="old('card_image', $city->card_image)" placeholder="cities/agra-card.jpg" />
            </div>
            <div>
                <x-admin-image-field name="banner_image" label="Banner Image" :value="old('banner_image', $city->banner_image)" placeholder="cities/agra-banner.jpg" />
            </div>
            <div>
            ...
            <div class="sm:col-span-2 flex gap-6">
                ...
                <label class="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" name="is_active" value="1" @checked(old('is_active', $city->is_active ?? true))> Active (visible on website)</label>
            </div>
        </div>
    ...
    @if ($city->exists)
        <div class="flex justify-end mt-4">
            <x-admin-delete-form :action="route('admin.master.cities.destroy', $city)" label="Delete City" />
        </div>
    @endif
@endsection
