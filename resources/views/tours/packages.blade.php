...
@section('content')
    ...
    <section class="relative pt-28 pb-14 sm:pb-16 bg-gradient-to-br from-brand-600 via-brand-700 to-ink overflow-hidden">
        <div class="absolute inset-0 opacity-20" style="background-image: radial-gradient(circle at 20% 50%, white 0%, transparent 40%);"></div>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex gap-3 overflow-x-auto pb-2 mb-8 -mx-1 px-1">
                    @foreach ($cat['monuments'] as $m)
                        <div class="shrink-0 w-36 sm:w-44 rounded-xl overflow-hidden border border-slate-100 bg-white shadow-sm">
                            <div class="aspect-[4/3] overflow-hidden">
                                <x-site-image :src="$m['image']" :alt="$m['name']" width="200" height="150" class="w-full h-full object-cover" />
                            </div>
                            <p class="text-[11px] font-bold text-ink p-2 leading-tight">{{ $m['name'] }}</p>
                        </div>
                    @endforeach
                </div>
                    ...
                    @foreach (array_slice($cat['tours'], 0, 3) as $package)
                        <article class="card-hover bg-white rounded-2xl overflow-hidden border border-slate-100 shadow-soft flex flex-col">
                            ...
                            <div class="p-4 flex flex-col flex-1">
                                ...
                                <h3 class="font-bold text-ink text-sm leading-snug mb-3 flex-1">{{ $package['title'] }}</h3>
                                <div class="flex items-center justify-between pt-3 border-t border-slate-100">
                                    <span class="text-lg font-extrabold text-brand">₹{{ $package['price'] }}</span>
                                    <a href="{{ TourCatalog::packageUrl($cat['slug'], $package['title']) }}" class="text-xs font-bold text-brand hover:text-brand-700">View Details →</a>
                                </div>
                            </div>
                        </article>
                    @endforeach
